...
Future<Recipe> fixImagePaths(Recipe recipe) async {
  ...
  if (recipe.imagePath != Constants.noRecipeImage) {
    newRecipeImagePath = await PathProvider.pP.getRecipeImagePathFull(
      _underscoreNewRecipeName,
      newRecipeImageDataType!,
    );
  ...
  for (int i = 0; i < recipe.stepImages.length; i++) {
    for (int j = 0; j < recipe.stepImages[i].length; j++) {
      ...
      recipeStepImages[i][j] =
          await PathProvider.pP.getRecipeStepNumberDirFull(
            _underscoreNewRecipeName,
            i,
          ) +
          "/" +
      ...
      if (recipe.stepImages[i][j] != recipeStepImages[i][j]) {
        String _recipeName = getRecipeNameOfStepImagePath(
          recipe.stepImages[i][j],
        );

        String newStepImagePreviewPath =
            await PathProvider.pP.getRecipeStepPreviewNumberDirFull(
              _underscoreNewRecipeName,
              i,
            ) +
            "/p-$stepImageFileName";
        String oldStepImagePreviewPath =
            await PathProvider.pP.getRecipeStepPreviewNumberDirFull(
              _recipeName,
              i,
            ) +
...
Future<void> _cleanRecipeFiles(Recipe recipe) async {
  Directory recipeDir = Directory(
    await PathProvider.pP.getRecipeDirFull(recipe.name),
  );
...
Future<List<String>> _extractImagePaths(Recipe recipe) async {
  return []
    ...
    ..addAll(
      (await PathProvider.pP.getRecipeStepPreviewPathList(
        recipe.stepImages,
        recipe.name,
      )).expand((i) => i).toList(),
    );
...
Future<void> deleteRecipeData(String recipeName) async {
  Directory recipeDir = Directory(
    await PathProvider.pP.getRecipeDirFull(recipeName),
  );
    ...
    for (FileSystemEntity f in recipeFiles) {
      if (f is File && f.path.contains('/stepImages/')) {
        ...
        File oldRecipeImageFile = File(
          await PathProvider.pP.getRecipeImagePathFull(oldRecipeName, dataType),
        );
        await oldRecipeImageFile.copy(
          await PathProvider.pP.getRecipeImagePathFull(newRecipeName, dataType),
        );

        File oldRecipePreviewImageFile = File(
          await PathProvider.pP.getRecipeImagePreviewPathFull(
            oldRecipeName,
            dataType,
          ),
        );
        await oldRecipePreviewImageFile.copy(
          await PathProvider.pP.getRecipeImagePreviewPathFull(
            newRecipeName,
            dataType,
          ),
...
Future<void> saveRecipeImage(
  ...
  String recipeImagePreviewPathFull = await PathProvider.pP
      .getRecipeImagePreviewPathFull(recipeName, dataType);
...
Future<void> deleteRecipeImageIfExists(String recipeName) async {
  ...
  if (await recipeDir.exists()) {
    ...
    Directory recipePreviewDir = Directory(
      await PathProvider.pP.getRecipePreviewDirFull(recipeName),
    );
    if (recipePreviewDir.existsSync()) {
...
Future<void> deleteStepImage(
  ...
  PathProvider.pP.getRecipeStepNumberDirFull(recipeName, stepNumber).then((
    path,
  ) {
...
Future<List<String>> getBackupedRecipenames() async {
  ...
  List importFiles = (await PathProvider.pP.getExternalAppDir()).listSync(
    recursive: true,
  );

  for (FileSystemEntity f in importFiles as Iterable<FileSystemEntity>) {
    recipeFileNames.add(
      f.path.substring(f.path.lastIndexOf("/") + 1, f.path.length - 4),
    );
  }
...
Future<String> saveRecipeZip(
  ...
  if (recipeDir.existsSync()) {
    await encoder.addDirectory(recipeDir);
  }
  await encoder.close();
...
Future<Recipe?> importMRBrecipeFromTmp(String recipeName) async {
  ...
  for (FileSystemEntity f in importFiles as Iterable<FileSystemEntity>) {
    if (f.path.endsWith('.xml')) {
      ...
      Tuple2<Recipe, String?> recipeData = getSpecifiedRecipeFromMRB(
        xmlData,
        recipeName,
      )!;

      Recipe finalRecipe = recipeData.item1;
      File recipeImageFile = File(
        importDir.path + "images/${recipeData.item2}",
      );
      ...
      if (recipeImageFile.existsSync()) {
        ...
        finalRecipe = finalRecipe.copyWith(
          imagePath: await PathProvider.pP.getRecipeImagePathFull(
            recipeName,
            getImageDatatype(recipeData.item2!)!,
          ),
...
Future<String> saveStepImage(
  ...
  await saveImage(newImage, stepImagePathFull, false);
  await saveImage(
    newImage,
    await PathProvider.pP.getRecipeStepPreviewNumberDirFull(
          recipeName,
...
Future<void> saveImage(File image, String targetPath, bool preview) async {
  ...
  if (image.path.endsWith(".jpg") ||
    ...
    print('saved images to the following directory');
    print(image.path);
    print(targetPath);
  }
}
...
Future<Map<String, Recipe?>> importRecipesToTmp(
  ...
  if (importMulitiple) {
    ...
    Map<String, Recipe?> importedRecipes = await importRecipeToTmp(
      recipeZip,
      keepTime,
    );
...
Future<Map<String, Recipe?>> importRecipeToTmp(
  File recipeZip,
  bool keepTime, {
  String? locale,
}) async {
  ...
  for (FileSystemEntity file in importFiles as Iterable<FileSystemEntity>) {
    ...
    if (file.path.endsWith('.json')) {
      try {
        importRecipe = await getRecipeFromJson(
          file as File,
          keepTime: keepTime,
        );
      } catch (e) {
...
Future<List<Recipe>> importFirstStartRecipes(
  ...
  for (FileSystemEntity file in importFiles as Iterable<FileSystemEntity>) {
    ...
    if (file.path.endsWith('DE.json')) {
      Recipe imagePathRecipe = Recipe.fromMap(
        jsonDecode(await (file as File).readAsString()),
      );
      Recipe importRecipe = Recipe.fromMap(
        jsonDecode(
          await File(
      ...
      String newRecipeFileName = importRecipe.imagePreviewPath.substring(
        1,
        importRecipe.imagePreviewPath.indexOf("/", 1),
      );
      ...
      if (locale != "DE") {
        ...
        for (FileSystemEntity stepFile in Directory(
          importPath + recipeFileName,
        ).listSync(recursive: true)) {
          if (stepFile is File) {
            Directory newStepDir = Directory(
              stepFile.path
          ...
          imagePath: importRecipe.imagePath != Constants.noRecipeImage
              ? await PathProvider.pP.getRecipeImagePathFull(
                  ...
                  importRecipe.imagePath.substring(
                    importRecipe.imagePath.lastIndexOf("."),
                  ),
                )
...
bool checkIfImportRecipeDataIsValid(
  Recipe importRecipe,
  Directory importRecipeDir,
) {
  String recipeDirName = importRecipeDir.path.substring(
    importRecipeDir.path.lastIndexOf("/"),
  );
  ...
  try {
    if (importRecipe.imagePath != Constants.noRecipeImage) {
      File importRecipeImageDir = File(
        importRecipeDir.path +
            importRecipe.imagePath.substring(
              importRecipe.imagePath.indexOf(recipeDirName) +
                  recipeDirName.length,
            ),
      );
      File importRecipeImagePreviewDir = File(
        importRecipeDir.path +
            importRecipe.imagePreviewPath.substring(
    ...
    for (int i = 0; i < importRecipe.stepImages.length; i++) {
      for (String stepImage in importRecipe.stepImages[i]) {
        File stepImageFile = File(
          importRecipeDir.path +
              stepImage.substring(
                stepImage.indexOf(recipeDirName) + recipeDirName.length,
              ),
        );
        File stepPreviewImageFile = File(
          importRecipeDir.path.substring(
                0,
                importRecipeDir.path.lastIndexOf("/"),
              ) +
              PathProvider.pP.getRecipeStepPreviewNumberDir(
                importRecipe.name,
                i,
              ) +
              "/p-" +
